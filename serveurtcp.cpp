...
unsigned int ServeurTcp::calculerCRC(unsigned char* reponse, int tailleTrame){
	
	unsigned int crc16KermitTable[256] = {
    ...
    {
        crc = (crc >> 8) ^ crc16KermitTable[(crc ^ reponse[i]) & 0xFF];
    }
    
    //std::cout<<hex<<"Le crc est: "<<(int)crc<<std::endl;
